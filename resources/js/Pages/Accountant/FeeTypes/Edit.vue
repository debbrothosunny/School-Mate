...
const props = defineProps({
    feeType: Object, // The fee type object to be edited
    frequencies: Array, // Array of available fee frequencies
});
...
const form = useForm({
    ...
    // Ensure status is a number (0 or 1) to match option values
    status: Number(props.feeType.status),
});
...
const submit = () => {
    ...
    form.post(route('fee-types.update', props.feeType.id), {
        onSuccess: () => {
            // No need to reset form, it will be re-rendered with updated data
        },
        onError: (errors) => {
            console.error("Error updating fee type:", errors);
        },
    });
};
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Fee Type: {{ feeType.name }}</h2>
        </template>
