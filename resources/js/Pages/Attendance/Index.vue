...
const attendanceForm = useForm({
  ...
  session_id: filterForm.session_id, // Added for completeness for backend submission
  section_id: filterForm.section_id, // Added for completeness for backend submission
...
const fetchStudents = () => {
  // Now includes group_id as a REQUIRED selection criterion
  if (filterForm.class_id && filterForm.session_id && filterForm.section_id && filterForm.group_id && filterForm.date) {
    ...
    attendanceForm.attendance_data = [];
    currentMessage.value = { text: 'Please select Class, Session, Section, Group, and Date to view students.', type: 'info' }; // Updated message
    showStudentList.value = false;
  }
};
...
watch([
  ...
  filterForm.group_id, // Watch group_id (now required)
  filterForm.date
], (newValues, oldValues) => {
...
onMounted(() => {
  ...
  currentMessage.value = { text: 'Please select Class, Session, Section, Group, and Date, then click "Filter Students" to view data.', type: 'info' }; // Updated message
});


// --- Save Attendance ---
const saveAttendance = () => {
  // The 'attendanceForm' already contains class_id, date, and attendance_data
  ...
  <AuthenticatedLayout>
    <div class="container mx-auto p-6 bg-gray-100 min-h-screen font-[Inter]">
      <div class="bg-white p-8 rounded-lg shadow-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div>
            ...
            <select
              ...
              :disabled="filterForm.processing"
            >
              <option :value="null" disabled>-- Select Group --</option> <!-- Now disabled if null -->
        ...
        <!-- NEW: Filter Students Button -->
        <div class="mt-6 mb-8 text-center">
          <button
            @click="fetchStudents"
            :disabled="filterForm.processing || !filterForm.class_id || !filterForm.session_id || !filterForm.section_id || !filterForm.group_id || !filterForm.date"
            :class="{ 'opacity-50 cursor-not-allowed': filterForm.processing || !filterForm.class_id || !filterForm.session_id || !filterForm.section_id || !filterForm.group_id || !filterForm.date }"
        ...
        <!-- Message if not all required filters are selected initially or before filter is clicked -->
        <div v-else-if="!showStudentList && (!filterForm.class_id || !filterForm.session_id || !filterForm.section_id || !filterForm.group_id || !filterForm.date)" class="text-center py-8 text-gray-600">
          Please select all required filters (Class, Session, Section, Group, and Date) and click "Filter Students" to view data.
        </div>
                ...
                <tr v-for="student in attendanceForm.attendance_data" :key="student.student_id">
                  <td class="px-6 py-4 whitespace-nowrap">
                    <div class="text-sm font-medium text-gray-900">
                      <!-- Find student name from props.students as attendanceForm.attendance_data only has id and status -->
