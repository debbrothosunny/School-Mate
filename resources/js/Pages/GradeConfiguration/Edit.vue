...
import { ref } from 'vue'; // Import ref if not already there, for consistency

const props = defineProps({
    gradeConfiguration: Object, // The specific grade configuration being edited
    flash: Object,
});

const form = useForm({
    class_interval: props.gradeConfiguration.class_interval,
    letter_grade: props.gradeConfiguration.letter_grade,
    grade_point: props.gradeConfiguration.grade_point,
    // ✨ FIX HERE: Explicitly convert boolean status to number ✨
    status: props.gradeConfiguration.status ? 1 : 0,
});

const submit = () => {
    form.post(route('grade-configurations.update', props.gradeConfiguration.id), {
        onError: (errors) => {
            console.error('Edit errors:', errors);
        },
        // Optional: onFinish or onSuccess to handle form reset/feedback
    });
};
...
<template>
    <Head :title="`Edit Grade: ${props.gradeConfiguration.letter_grade}`" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Grade Configuration</h2>
        </template>
