...
const props = defineProps({
  classes: Object, // Paginated data from Laravel
});
...
// Show SweetAlert pop-up on backend flash messages
watchEffect(() => {
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Class Teacher</th>
            ...
            <tbody v-if="filteredClasses.length" class="bg-white divide-y divide-gray-200">
              <tr v-for="(classItem, index) in filteredClasses" :key="classItem.id">
                ...
                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-500">{{ classItem.teacher ? classItem.teacher.name : 'N/A' }}</td>


                <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <Link :href="route('class-names.edit', classItem.id)" class="text-indigo-600 hover:text-indigo-900 mr-4">Edit</Link>
                  <button @click="confirmClassDeletion(classItem)" class="text-red-600 hover:text-red-900 cursor-pointer">Delete</button>
                </td>
              </tr>
            </tbody>
            <tbody v-else>
              <tr>
                <td colspan="6" class="px-4 py-4 text-center text-sm text-gray-500">No classes found.</td>
              </tr>
            </tbody>
          </table>
        </div>
