      ...
      <div class="card shadow-sm rounded-lg">
        <div class="card-body p-4">
          <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <Link :href="route('class-subjects.create')" class="btn btn-dark btn-sm rounded">
          ...
          <div class="table-responsive">
            <table class="table table-hover align-middle">
              <thead class="table-light">
                <tr>
                  <th scope="col" class="px-3 py-3 text-uppercase text-muted small">S/N</th>
              ...
              <tbody class="bg-white" v-if="filteredClassSubjects.length">
                <tr v-for="(cs, index) in filteredClassSubjects" :key="cs.id">
                  <td class="px-3 py-4 text-nowrap">
                    {{
                      ((props.classSubjects.current_page - 1) * props.classSubjects.per_page) +
                        index +
                        1
                    }}
                  </td>
                  <td class="px-3 py-4 text-nowrap">{{ cs.class_name?.class_name || 'N/A' }}</td>
              ...
              <tbody v-else>
                <tr>
                  <td colspan="9" class="text-center py-4 text-muted">No class subjects found.</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div
            ...
            v-if="!searchQuery && props.classSubjects.data.length > 0"
          >
            ...
            <nav aria-label="Page navigation" v-if="classSubjects.links && classSubjects.links.length > 3">
              <ul class="pagination mb-0">
                <li
                  v-for="link in classSubjects.links"
                  :key="link.label"
                  class="page-item"
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          ...
          <div class="modal-body">
            ...
            for Session:
            <span class="fw-bold">{{ classSubjectToDelete?.session?.name || '' }}</span>
            and Section:
            <span class="fw-bold">{{ classSubjectToDelete?.section?.name || '' }}</span>
            <span v-if="classSubjectToDelete?.group?.name">
              and Group:
              <span class="fw-bold">{{ classSubjectToDelete?.group?.name || '' }}</span>
            </span>
