...
const marksForm = useForm({
  ...
  marks_data: props.students.map(student => ({
    ...
    class_test_marks: student.class_test_marks ?? null,
    assignment_marks: student.assignment_marks ?? null,
    exam_marks: student.exam_marks ?? null,
  })),
});
  ...
  <AuthenticatedLayout>
    ...
    <div class="py-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <section v-if="filterForm.exam_id && props.selectedExamTotalMarks > 0" class="mt-8 p-4 bg-indigo-50 border border-indigo-300 text-indigo-900 rounded-md">
        ...
        <p class="mt-3 font-bold text-indigo-700 text-sm">Total must not exceed 50 marks.</p>
      </section>
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-100">
                <tr>
                  ...
                  <th class="px-6 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider">Class Test (5)</th>
                  <th class="px-6 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider">Assignment (5)</th>
                  <th class="px-6 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider">Exam</th>
                  <th class="px-6 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider">Attendance (10)</th>
                </tr>
              </thead>
                ...
                <tr v-for="(student, index) in props.students" :key="student.id" class="hover:bg-indigo-50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-center">
                    <input
                      ...
                      v-model="marksForm.marks_data[index].class_test_marks"
                      :name="`marks_data[${index}][class_test_marks]`"
                      class="w-20 rounded-md border border-gray-300 text-center text-sm shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50"
                    />
                    <p v-if="marksForm.errors[`marks_data.${index}.class_test_marks`]" class="text-xs text-red-600 mt-1">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-center">
                    <input
                      type="number"
                      step="0.01" min="0" max="5"
                      v-model="marksForm.marks_data[index].assignment_marks"
                      :name="`marks_data[${index}][assignment_marks]`"
                      class="w-20 rounded-md border border-gray-300 text-center text-sm shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50"
                    />
                    <p v-if="marksForm.errors[`marks_data.${index}.assignment_marks`]" class="text-xs text-red-600 mt-1">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-center">
                    <input
                      type="number"
                      min="0" :max="props.selectedExamTotalMarks"
                      v-model="marksForm.marks_data[index].exam_marks"
                      :name="`marks_data[${index}][exam_marks]`"
                      class="w-20 rounded-md border border-gray-300 text-center text-sm shadow-sm focus:border-indigo-500 focus:ring focus:ring-indigo-300 focus:ring-opacity-50"
                    />
                    <p v-if="marksForm.errors[`marks_data.${index}.exam_marks`]" class="text-xs text-red-600 mt-1">
                      {{ marksForm.errors[`marks_data.${index}.exam_marks`] }}
                    </p>
                    <p v-if="marksForm.errors[`marks_data.${index}.total_marks`]" class="text-xs text-red-600 mt-1">
